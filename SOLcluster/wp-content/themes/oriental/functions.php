...
function oriental_post_image_html( $html, $post_id, $post_image_id ) {
	...
	$thumbnail_url = "";
	$image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id($post_id) ); // returns an array
	if( $image_attributes ) {
		$thumbnail_url = $image_attributes['0'];
	}

	$html = '<div class="oriental-post-thumbnail" style="background: url(' . $thumbnail_url . ') no-repeat center center; background-size: cover"><div class="oriental-post-thumbnail-inner">' . $html . '</div></div>';
